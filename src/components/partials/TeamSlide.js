    ...
    <div className={`inner ${props.slide.title2 ? 'reverse' : ''}`}>
        ...
        <div className="image-group">
          {props.slide.images.map((image, key) => {
            return (
              <div
                key={key}
                className={`image-item ${image.half ? 'half' : 'full'}`}
              >
                <div
                  className="image"
                  title={image.caption}
      ...
      <div className="caption-wrapper">
        <div className="wrapper">
          ...
          <h6
            className="subtitle serif"
            dangerouslySetInnerHTML={{ __html: props.slide.subtitle }}
          />
          <p
            className="serif"
            dangerouslySetInnerHTML={{ __html: props.slide.para }}
          />
        </div>
      </div>
      {props.slide.title2 ? (
        <div className="caption-wrapper second-team">
          <div className="wrapper">
            ...
            <p
              className="serif"
              dangerouslySetInnerHTML={{ __html: props.slide.para2 }}
            />
          </div>
        </div>
      ) : (

...
export default class Availability extends Component {
  ...
  componentWillMount() {
    if (window.location.origin === 'https://108leonard.com') {
      window.gtag('config', 'UA-113369414-1', {
        page_title:
          '108 Leonard | Available apartments for sale in Tribeca NYC.',
  ...
  componentDidMount() {
    ...
    if (document.getElementsByTagName('meta').description) {
      document.getElementsByTagName('meta').description.content =
        'Please contact the sales gallery for current availability.';
  ...
  loadResidences() {
    ...
    fetch(
      'https://residences.api.dbxd.com/getunits/available?projectname=108leonard'
    )
      ...
      .then(json => {
        ...
        for (var i in json) {
          if (json[i]) {
            ...
            let number = parseInt(
              json[i]['unit_num'].split(' (')[0].match(/(\d+|[^\d]+)/g)[0],
              10
            );
            let letter =
              json[i]['unit_num'].indexOf('PH') > -1
                ? json[i]['unit_num'].split(' (')[0]
            ...
            switch (letter) {
              ...
              case 'D':
                if (number >= 3 && number <= 5) {
                  residenceSVGFileName = 'D_3-5';
                  residencePDFFileName = 'D_3-5';
                } else if (number >= 7 && number <= 8) {
                  residenceSVGFileName = 'D_7-8';
                  residencePDFFileName = 'D_7-8';
                } else if (number >= 9 && number <= 11) {
                  residenceSVGFileName = 'D_9-11';
                  residencePDFFileName = 'D_9-11';
                } else if (number === 6) {
              ...
              case 'F':
                if (number >= 2 && number <= 12) {
                  residenceSVGFileName = 'F_2-12';
                  residencePDFFileName = 'F_2-12';
                } else {
              ...
              case 'K':
                if (number >= 4 && number <= 11) {
                  residenceSVGFileName = 'K_4-11';
                  residencePDFFileName = 'K_4-11';
                }
                break;
              case 'L':
                if (number >= 4 && number <= 6) {
                  ...
                  residenceSVGFileName = 'L_6-11';
                  residencePDFFileName = 'L_6-11';
                } else {
              ...
              case 'M':
                if (number >= 4 && number <= 11) {
                  residenceSVGFileName = 'M_4-11';
                  residencePDFFileName = 'M_4-11';
                } else {
              ...
              case 'P':
                if (number >= 7 && number <= 9) {
                  residenceSVGFileName = 'P_7-9';
                  residencePDFFileName = 'P_7-9';
            ...
            parsed[i] = {
              ...
              baths: parseFloat(
                json[i]['unit_type'].split('/')[1].split('BA')[0]
              ),
        ...
        let bedSorted = parsed.sort(function(a, b) {
          return a['bedrooms'] - b['bedrooms'];
        });
        let priceSorted = bedSorted.sort(function(a, b) {
    ...
    setTimeout(() => {
      ...
      if (residences === 'penthouses') {
        filterRes = el => {
          if (
            el['number'] > 13 ||
            el.residence.indexOf('EAST') > -1 ||
            el.residence.indexOf('NORTH') > -1
          ) {
            return true;
          } else {
            return false;
        ...
        newFilteredArray = newFilteredArray.concat(
          this.state.residences.filter(filterRes)
        );
  ...
  onFilterColumn(filter, arrow) {
    ...
    // sort DESC
    if (arrow === 'up') {
      newSort = this.state.residences.sort(function(a, b) {
        if (filter === 'residence') {
  ...
  onFilterItem(filter, checked) {
    if (checked) {
      ...
      this.setState({
        filtersArray: this.state.filtersArray.filter(function(i) {
          return i !== filter;
        })
      });
    }
    ...
    return (
      <div className="availability-page">
        <div className="filter-button-wrapper">
          <Button
            btnEl={el => (this.btnElement = el)}
            name="Filter"
            onClick={this.onFilterClick}
            idClass="filter-button"
          />
          <Button
            name="Share"
            disabled={this.state.disabledShare}
        ...
        <div className="list-wrapper">
          <Filter
            ...
            allResidences={this.state.allResidences}
            sendResidences={this.sendResidences.bind(this)}
            filtersArray={this.state.filtersArray}
            onFilterItem={this.onFilterItem.bind(this)}
            onFilterColumn={this.onFilterColumn.bind(this)}
          />
          <List
            listElement={el => (this.listElementRef = el)}
            residences={this.state.residences}
            onViewFloorplanClick={this.onViewFloorplanClick.bind(this)}
            sendCheckboxes={this.sendCheckboxes.bind(this)}
          />
        </div>
        <ScrollArrow
          ref={i => {
          ...
          listElementRef={this.listElementRef}
        />

        {/* TOOD: try to move floorplan to here */}
        <VelocityComponent
          ref={e => {
            this.floorplanwrapper = e;
          }}
          duration={500}
        ...
        >
          <Floorplan
            ...
            intft={this.delimitNumbers(
              this.state.floorplanResidenceArray.interior
            )}
            intsqm={this.sqmFormat(this.state.floorplanResidenceArray.interior)}
            hasext={
              this.state.floorplanResidenceArray.exterior > 1 ? true : false
            }
            extft={this.delimitNumbers(
              this.state.floorplanResidenceArray.exterior
            )}
